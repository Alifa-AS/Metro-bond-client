...
const SuccessStoryDetails = () => {
  const { coupleImage, reviewStar, marriageDate, successStory, brideName, groomName, extraStory } =
    useLoaderData();

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-pink-100 to-purple-100 p-10">
      <div className="bg-white shadow-2xl rounded-3xl p-10 max-w-2xl text-center backdrop-blur-lg bg-opacity-90 border border-pink-300 relative overflow-hidden">
        ...
        {/* Couple Names */}
        <h2 className="text-2xl font-semibold text-gray-800 mt-4">{brideName} & {groomName}</h2>
        ...
        <div className="flex justify-center my-6 relative z-10">
          <Rating
            ...
            value={reviewStar}
            readOnly
            className="scale-110 transform hover:scale-125 transition-all duration-300"
          />
        </div>
        ...
        {extraStory && (
                    <div className="mt-6 p-4 bg-pink-50 border-l-4 border-pink-400 rounded-md shadow-md">
                        <h3 className="text-lg font-semibold text-pink-600">More About {brideName} & {groomName}</h3>
